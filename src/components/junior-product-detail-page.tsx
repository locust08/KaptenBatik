...
export function JuniorProductDetailPage({ product }: { product: JuniorProductDetail }) {
  const isGamelanPage = product.slug === "maharaja-junior-gamelan-golden-cream";
  const isIlhamMusePage = product.slug === "maharaja-junior-ilham-muse-picasso-lily";
  const isCapriWindsurferPage =
    product.slug === "nusantara-junior-capri-mount-kinabalu-windsurfer";
  const hasLiftedModel =
    isIlhamMusePage ||
    isGamelanPage ||
    product.slug === "nusantara-junior-saxosoul-blue-glow";
  ...
  const reviewCards = buildProductReviewCards("junior", product.whyLove);
  const currentProductIndex = juniorProductSlugs.indexOf(product.slug);
  const previousProduct =
    juniorProductDetails[
      juniorProductSlugs[
        (currentProductIndex - 1 + juniorProductSlugs.length) % juniorProductSlugs.length
      ]
    ];
  const nextProduct =
    juniorProductDetails[
      juniorProductSlugs[(currentProductIndex + 1) % juniorProductSlugs.length]
    ];

  const buildCartItem = () => ({
    category: "Junior Batik",
    collection: "junior" as const,
    href: `/junior/${product.slug}`,
  ...
  useEffect(() => {
    setHasCollectionTransition(consumeCollectionTransition(`/junior/${product.slug}`));
  }, [product.slug]);
  ...
  return (
    <main
      ref={mainRef}
      className={`${styles.page} ${hasLiftedModel ? styles.liftedModelPage : ""} ${
        isGamelanPage ? styles.gamelanPage : ""
      } ${
        isIlhamMusePage ? styles.picassoLilyPage : ""
      } ${
        isCapriWindsurferPage ? styles.capriWindsurferPage : ""
      } ${
      ...
      <section className={styles.heroSection} id="product-detail" ref={heroSectionRef}>
        <div className={styles.heroWrap}>
          <div className={styles.heroVisual}>
            <Link className={styles.backLink} href="/?collection=junior#collection">
              &lt; Back
            </Link>
          ...
          <button
            ...
            onClick={() => handleDetailSelect(`/junior/${previousProduct.slug}`)}
            type="button"
          >
          ...
          <button
            ...
            onClick={() => handleDetailSelect(`/junior/${nextProduct.slug}`)}
            type="button"
          >
            <span className={styles.heroNavCopy}>
      ...
      <section className={`${styles.reviewSection} ${styles.scrollRevealRoot}`} data-reveal-root="">
        <div className={styles.reviewHeader}>
          ...
          <p className={`${styles.reviewSummary} ${styles.revealItem}`} data-reveal-item="">
            A quick read on comfort, movement, and the finish of the cloth.
          </p>
        </div>
      ...
      <section className={`${styles.related} ${styles.scrollRevealRoot}`} data-reveal-root="">
        <div className={styles.relatedHeader}>
          ...
          <Link
            ...
            data-reveal-item=""
            href="/?collection=junior#collection"
          >
            View All
          </Link>
        </div>
        ...
        <div className={styles.relatedGrid}>
          {product.related.map((item) => (
            <Link
              ...
              data-reveal-item=""
              href={`/junior/${item.slug}`}
              key={item.slug}
              onClick={(event) => {
                event.preventDefault();
                handleDetailSelect(`/junior/${item.slug}`);
              }}
            >
      ...
      <section className={styles.cta}>
        <div className={`${styles.ctaPanel} ${styles.scrollRevealRoot}`} data-reveal-root="">
          ...
          <div className={`${styles.ctaButtons} ${styles.revealItem}`} data-reveal-item="">
            ...
            <Link className={styles.ctaSecondary} href="/?collection=junior#collection">
              Browse More
            </Link>
          </div>
        </div>
