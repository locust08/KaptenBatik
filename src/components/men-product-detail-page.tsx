...
export function MenProductDetailPage({ product }: { product: MenProductDetail }) {
  const isBloomcyclePage = product.slug === "maharaja-slim-fit-bloomcycle-crystal-pink";
  const isIlhamPage = product.slug === "maharaja-slim-fit-ilham-muse-lime-blue";
  const isSyandanaPage = product.slug === "maharaja-linen-syandana-aquamarine";
  const isEnduraPage = product.slug === "endura-canvas-jacket-forest-heart";
  const hasLiftedModel = isBloomcyclePage || isIlhamPage || isEnduraPage;
  ...
  const reviewCards = buildProductReviewCards("men", product.whyLove);
  const currentProductIndex = menProductSlugs.indexOf(product.slug);
  const previousProduct =
    menProductDetails[
      menProductSlugs[
        (currentProductIndex - 1 + menProductSlugs.length) % menProductSlugs.length
      ]
    ];
  const nextProduct =
    menProductDetails[menProductSlugs[(currentProductIndex + 1) % menProductSlugs.length]];

  const buildCartItem = () => ({
    category: "Men's Batik",
    collection: "men" as const,
  ...
  useEffect(() => {
    setHasCollectionTransition(consumeCollectionTransition(`/men/${product.slug}`));
  }, [product.slug]);
  ...
  return (
    <main
      ref={mainRef}
      className={`${styles.page} ${isBloomcyclePage ? styles.bloomcyclePage : ""} ${
        isIlhamPage ? styles.ilhamPage : ""
      } ${isSyandanaPage ? styles.syandanaPage : ""} ${
        isEnduraPage ? styles.enduraPage : ""
      ...
      <section className={styles.heroSection} id="product-detail" ref={heroSectionRef}>
        <div className={styles.heroWrap}>
          <div className={styles.heroVisual}>
            <Link className={styles.backLink} href="/#collection">
              &lt; Back
            </Link>
          ...
          <button
            ...
            onClick={() => handleDetailSelect(`/men/${previousProduct.slug}`)}
            type="button"
          >
          ...
          <button
            ...
            onClick={() => handleDetailSelect(`/men/${nextProduct.slug}`)}
            type="button"
          >
            <span className={styles.heroNavCopy}>
      ...
      <section className={`${styles.reviewSection} ${styles.scrollRevealRoot}`} data-reveal-root="">
        <div className={styles.reviewHeader}>
          ...
          <p className={`${styles.reviewSummary} ${styles.revealItem}`} data-reveal-item="">
            A quick read on fit, fabric, and the overall feel of the piece.
          </p>
        </div>
          ...
          {product.related.map((item) => (
            <Link
              ...
              onClick={(event) => {
                ...
                handleDetailSelect(`/men/${item.slug}`);
              }}
            >
